...
// import { ShoppingCart, CheckCircle2, Search, Calculator, Building, PieChart, Ruler, Settings, Scale, BookOpen, Loader2, AlertCircle } from 'lucide-react';
...
//   // 1. Available courses + derive maxCourses
//   useEffect(() => {
...
//       const list = data.SelectedCourses ?? data.selectedCourses ?? [];
//       setSelectedCourses(list.map(mapSelectedCourse));
//     } catch (_) {}
...
//           alert(err.message ?? `Failed to remove course (${res.status})`);
//           return;
//         }
...
//     // Block if limit reached — message reflects GPA reason
//     if (selectedCourses.length >= maxCourses) {
//       const gpa = studentInfo?.gpa;
//       const msg = (!gpa || gpa === 0)
//         ? `Maximum ${maxCourses} courses allowed.`
//         : gpa >= 2
//           ? `Maximum ${maxCourses} courses allowed (GPA > 2).`
//           : `Maximum ${maxCourses} courses allowed because your GPA (${gpa}) is ≤ 2.\nImprove your GPA to unlock 6 courses.`;
//       alert(msg);
//       return;
//     }
...
//       if (!res.ok) {
//         const err = await res.json().catch(() => ({}));
//         alert(err.message ?? `Failed to select course (${res.status})`);
//         return;
//       }

//       const result = await res.json();

//       // Update maxCourses from backend response
...
//                 {remainingSlots > 0 ? `${remainingSlots} slot${remainingSlots > 1 ? 's' : ''} remaining` : '🔒 Selection full'}
//                 {gpaLabel && ` · ${gpaLabel}`}
//               </span>
//             </div>
...
//               <input
//                 type="text" placeholder="Filter your enrolled courses..."
...
function mapEnrolledCourse(c) {
  ...
  const dept = c.DepartmentName ?? c.departmentName ?? '';
  const credits = c.CreditHours ?? c.creditHours ?? 3;
  const passed = c.Passed ?? c.passed ?? null;
  return {
    id, code: id, title: name,
...
const CourseCard = ({ course, enrolled, selected, onToggle, onRemove, mode }) => {
  ...
  const passedBadge = mode === 'enrolled' && course.passed !== null && course.passed !== undefined
    ? course.passed === true
      ? <span className="mt-1 inline-block rounded-[6px] bg-[#d1fae5] px-2 py-0.5 text-[10px] font-[700] text-[#059669]">Passed ✔</span>
      : course.passed === false
        ? <span className="mt-1 inline-block rounded-[6px] bg-[#fee2e2] px-2 py-0.5 text-[10px] font-[700] text-[#ef4444]">Failed ✗</span>
        : <span className="mt-1 inline-block rounded-[6px] bg-[#fef3c7] px-2 py-0.5 text-[10px] font-[700] text-[#d97706]">In Progress</span>
    : null;
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-wrap justify-center gap-3">
              {[
                ...
                { label: 'Passed', value: enrolledCourses.filter(c => c.passed === true).length,  color: '#059669' },
                { label: 'Failed', value: enrolledCourses.filter(c => c.passed === false).length, color: '#ef4444' },
              ].map(s => (
