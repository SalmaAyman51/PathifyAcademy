...
// export default function InternalProfessor() {
...
//           fetch(`${API}/api/Professor/internal/supervised-teams`, {
//             headers: { Authorization: `Bearer ${token}` }
//           }),
//           fetch(`${API}/api/Professor/internal/pending-proposals`, {
//             headers: { Authorization: `Bearer ${token}` }
//           })
//         ]);
...
//     try {
//       const res = await fetch(`${API}/api/Professor/review-proposal-internal/${proposalId}`, {
//         method: 'POST',
//         headers: {
...
//                   <div className="text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded w-max">Active Team</div>
...
export default function InternalProfessor() {
  ...
  // جلب البيانات الأساسية (الفرق والمقترحات المعلقة)
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      setError(null);
  ...
  return (
    <div className="space-y-6">
      {/* الإحصائيات مع إضافة إحصائية الـ Approved Projects */}
      ...
      {/* شريط التبويبات العلوي بعد إضافة All Projects */}
      <div className="flex border-b border-slate-200 gap-6 text-[13px] font-semibold text-slate-400 items-center justify-between">
        ...
        {/* خانة البحث تظهر فقط عند اختيار تبويب All Projects */}
        {currentView === 'all' && (
